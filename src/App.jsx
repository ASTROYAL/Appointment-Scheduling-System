...
// Enhanced Error Handling and Network Utilities
...
class ConcurrencyError extends AppointmentError {
  constructor(message, details = {}) {
    super(message, 'concurrency', details);
...
const retryWithBackoff = async (fn, maxRetries = 3, baseDelay = 1000) => {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Don't retry validation or other errors
      throw error;
    }
  }
...
const AppointmentManagementView = () => {
  ...
  const loadDashboardMetrics = async () => {
    try {
      ...
      // Don't show error for metrics failure
    }
  };
  ...
  const loadAppointments = async () => {
    ...
    try {
      const result = await retryWithBackoff(async () => {
        ...
        const appointmentData = await getAppointments(currentFilters);
      ...
      } else if (parsedError instanceof ConcurrencyError) {
        errorMessage = `Concurrent operation detected: ${parsedError.message}. Please refresh and try again.`;
      } else {
  ...
  const handleCreateAppointment = async (appointmentData) => {
    try {
      ...
      }, 1, 1000); // Only 1 retry for creation
  ...
  const handleUpdateAppointmentStatus = async (appointmentId, newStatus) => {
    ...
    try {
      ...
      await retryWithBackoff(async () => {
        return await updateAppointmentStatus(appointmentId, newStatus);
      }, 2, 1000);
      ...
      if (parsedError instanceof NetworkError) {
        ...
        setError(`Concurrent update detected: ${parsedError.message}. Please refresh and try again.`);
      } else {
